...
def getsavedhashrate():
    while True:
        svh = open("Config\hashrate.txt", "r")
        lvh = svh.read()
        try:
...
class SiriCoinMiner(object):
    ...
    def printBalance(self):
        ...
        print(f"[blue]Your SiriCoin Balance {balance}[/blue]")
            ...
            try:
                ...
                while True:
                    ...
                    if (self_lastBlock != self.lastBlock):
                        ...
                        URL = 'https://node-1.siricoin.tech:5006/'
                        req = requests.get(URL+"stats")
                        res = json.loads(req.text)
                        currentnetdiff = res['result']['chain']['difficulty']
                        nonfrmtdiff = diffformat(currentnetdiff)
                    ...
                    while (time.time() - tinicial) < (temptime * 2):
                        if (ser.in_waiting>0):
                            ...
                            if (byte_lido == b'\n'):
                                ...
                                savehashrate = self.formatHashrate((self.nonce / tempo_decorrido))
                                with open("Config\hashrate.txt", "w") as f:
                                    f.write(f"{savehashrate}")
