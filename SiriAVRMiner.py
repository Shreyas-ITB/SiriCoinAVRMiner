...
def startMining():
    ...
    """)
    rpc = Presence("941983994646515713")
    rpc.connect()
    rpc.update(state="Getting Rich Overnight", details="Mining with mSiris AVR", large_image="logo", start=time.time())
    sleep(3)
    ...
    while True:
        try:
            ...
            sleep(2)
            try:
                req = requests.get(f"http://138.197.181.206:8000/createData/{wallet}")
                ...
                try:
                    ...
                    sleep(25)
                    avrdata = requests.get(f"http://138.197.181.206:8000/acceptjob/{cleanedjob}/{wallet}")
